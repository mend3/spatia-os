...
const { entityPhysics, classificar, fenomenos, dominanteDe } = await import(`${RAIZ}/src/space/entity-physics.js`);
...
function ontologia(node, injecao = {}) {
  ...
  const ctx = { dominante: dominantes.has(node.id), sistema: node.dir || node.repo || '', ...(injecao.ctx || {}) };
  const extra = injecao.pos || [];
  const fisica = entityPhysics(noNo, ctx, ...extra);
  const classe = classificar(fisica, noNo, ...extra);
  const fens = fenomenos(fisica, noNo, ...extra).map((f) => f.tipo).sort();
  /*
...
console.log(`  corpus: ${CORPUS} · ${corpos.length} corpos · ${estruturas.length} estruturas · ${porSistema.size} sistemas`);
...
conferir('§1 todo contexto declarado tem entrada em APARENCIAS',
  contextosDeclarados.every((c) => F.APARENCIAS[c] !== undefined),
  contextosDeclarados.filter((c) => F.APARENCIAS[c] === undefined).join(', '));
conferir('§1 APARENCIAS não declara contexto fora de CONTEXTO',
...
conferir('§1 toda aparência declara arquivo, origem e licença',
  todas.every((a) => a.arquivo && a.origem && a.licenca),
  todas.filter((a) => !(a.arquivo && a.origem && a.licenca)).map((a) => a.nome).join(', '));
conferir('§1 nenhum arquivo serve a duas aparências',
...
conferir('§1 SEM_APARENCIA e APARENCIAS não se sobrepõem',
  casos.every((c) => !contextosDeclarados.includes(c)));
conferir('§1 todo motivo de SEM_APARENCIA é prosa, não rótulo',
...
conferir('§1 toda pele abre contexto ou tem caso nomeado com motivo', semDesfecho.length === 0,
  `sem desfecho: ${semDesfecho.join(', ')}`);
conferir('§1 estrutura tem caso próprio, separada do asteroide',
...
for (const [c, n] of populacao) {
  ...
  console.log(`  ${n ? '\x1b[32m✓\x1b[0m' : '\x1b[33m·\x1b[0m'} ${c.padEnd(12)} ${String(n).padStart(3)} corpos · ${aceitas} aparência(s) aceita(s)`);
}
...
const emDisco = new Set(todas.map((a) => a.arquivo)
  .filter((a) => typeof a === 'string' && fs.existsSync(path.join(RAIZ, a))));
console.log(`  \x1b[2m[medida] ${emDisco.size}/${todas.length} texturas do catálogo em disco (T-34 ainda não rodou)\x1b[0m`);
...
for (const arquivo of ['src/space/entity-physics.js', 'src/space/superficies.js']) {
  ...
  conferir(`§2 ${path.basename(arquivo)} não conhece favoritos`,
...
conferir('§2 favoritos.js só importa o vocabulário da pele',
  importsFav.join(',') === './superficies.js', importsFav.join(', ') || 'nenhum');
...
const VOCABULARIO_DA_MARCA = ['favorito', 'favoritos', 'favorites', 'marca', 'marcas', 'marcado',
  'marcados', 'aparencia', 'aparencias', 'APARENCIAS', 'CONTEXTO', 'contextoDe'];
let callSites = 0;
const contaminados = [];
...
conferir('§2 a varredura de call sites achou alguma coisa', callSites > 0,
 ...
 */
const marcaFalsa = { por: F.AUTORIA.OPERADOR, em: '2026-08-09T00:00:00.000Z', corpus: CORPUS, aparencias: { planetario: 'marte' } };
const canais = [
...
conferir('§2 marcar não muda física, classe, fenômeno nem pele de nenhum corpo',
  divergiram.length === 0, divergiram.slice(0, 3).join(' · '));
console.log(`  \x1b[2m${perturbacoes} perturbações em ${corpos.length} corpos · ${callSites} call sites varridos\x1b[0m`);
...
conferir('§3 "não marquei" é estado próprio',
...
conferir('§3 marcar sem escolher aparência é fato próprio',
...
conferir('§3 "marquei e a marca vale" traz nome e arquivo',
  valendo.estado === F.ESTADO.MARCADA && valendo.aparencia === 'marte'
...
conferir('§3 "marquei e não vale mais aqui" é o TERCEIRO estado',
  virouEstrela.estado === F.ESTADO.DEGRADADA && virouEstrela.marcada === true
  && virouEstrela.aparencia === null);
conferir('§3 a degradação ANUNCIA por que', /planetario/.test(virouEstrela.motivo));
conferir('§3 a escolha continua GRAVADA depois de degradar',
  virouEstrela.escolhas.planetario === 'marte');
conferir('§3 e ela VOLTA quando o corpo volta à classe de origem',
 ...
 */
conferir('§3 os dois nulos NÃO são o mesmo fato',
...
conferir('§3 sem lista de disco, `disponivel` é null — "não medi"',
...
conferir('§3 textura ausente degrada ANUNCIANDO, com disponivel false',
  semArquivo.estado === F.ESTADO.DEGRADADA && semArquivo.disponivel === false
  && /não está em disco/.test(semArquivo.motivo));
conferir('§3 com o arquivo em disco a escolha vale e disponivel é true',
  F.ler(e0, { id: ID, contexto: F.CONTEXTO.PLANETARIO, emDisco: new Set(['assets/textures/mars.jpg']) }).disponivel === true);

// O catálogo recusa o que não aceita, e o motivo NOMEIA o que aceita.
conferir('§3 escolher aparência de outro contexto é recusado',
  /Aceita:/.test(recusa(() => F.escolherAparencia(e0, { id: ID, contexto: F.CONTEXTO.PLANETARIO, aparencia: 'ceres' })) || ''));
conferir('§3 escolher em contexto que não aceita nada é recusado',
  Boolean(recusa(() => F.escolherAparencia(e0, { id: ID, contexto: F.CONTEXTO.NENHUM, aparencia: 'marte' }))));
conferir('§3 escolher aparência de corpo não marcado é recusado',
  Boolean(recusa(() => F.escolherAparencia(F.VAZIO, { id: ID, contexto: F.CONTEXTO.PLANETARIO, aparencia: 'marte' }))));
conferir('§3 ler sem contexto é recusado, nunca respondido por default',
  Boolean(recusa(() => F.ler(e0, { id: ID }))));
conferir('§3 desmarcar apaga a escolha junto',
  F.ler(F.desmarcar(e0, ID), { id: ID, contexto: F.CONTEXTO.PLANETARIO }).estado === F.ESTADO.NAO_MARCADO);
conferir('§3 esquecer aparência mantém a marca',
  F.ler(F.esquecerAparencia(e0, { id: ID, contexto: F.CONTEXTO.PLANETARIO }), { id: ID, contexto: F.CONTEXTO.PLANETARIO })
...
conferir('§4 chave não declarada é recusada pelo prefs',
  prefs.get('favoritos.inventado') === undefined);
...
conferir('§4 marcar e escolher atravessam o guardador', fav.marcados().join() === ID
  && fav.ler({ id: ID, contexto: F.CONTEXTO.PLANETARIO }).aparencia === 'jupiter');
...
conferir('§4 o valor sobrevive à ida e volta pelo JSON',
...
conferir('§4 o estado devolvido é congelado',
...
conferir('§5 marcar sem `corpus` é RECUSADO, com o conserto no motivo',
...
conferir('§5 autoria fora de AUTORIA é recusada, com o motivo do catálogo',
  /quem escolhe/.test(recusa(() => F.marcar(F.VAZIO, { id: ID, corpus: CORPUS, por: 'derivada' })) || ''));
conferir('§5 AUTORIAS_RECUSADAS não intersecta AUTORIA',
...
conferir('§5 o céu servido tem os três casos que a sonda precisa',
...
conferir('§5 a sonda diz POR QUEM e QUANDO em toda marca',
  relatorio.itens.every((i) => i.por === F.AUTORIA.OPERADOR && /^\d{4}-\d{2}-\d{2}T/.test(i.em)));
conferir('§5 a sonda ANUNCIA marca de outro céu em vez de escondê-la',
  relatorio.foraDoCeu === 1 && relatorio.itens.find((i) => i.foraDoCeu).corpus === 'outro_ceu');
conferir('§5 corpo fora da topologia sai com noCeu false, e a marca FICA',
  relatorio.ausentes === 1 && relatorio.total === 4);
conferir('§5 a PASTA é endereçável (só `id` sobrevive: nó de dir não tem `source`)',
  relatorio.itens.some((i) => i.id === umaPasta.id && i.noCeu === true));
conferir('§5 a estrela marcada sai com o motivo de SEM_APARENCIA da fotosfera',
  /textures\/sun\.jpg/.test(relatorio.itens.find((i) => i.id === umaEstrela.node.id).motivo));
conferir('§5 o planeta marcado degrada porque a textura não está em disco',
...
for (const i of relatorio.itens) {
  console.log(`  \x1b[2m${(i.estado || 'fora-do-ceu').padEnd(14)}\x1b[0m ${i.id.slice(-46).padEnd(46)} ${i.por} · ${i.em.slice(0, 10)}${i.foraDoCeu ? ` \x1b[33m· céu ${i.corpus}\x1b[0m` : ''}`);
}
...
if (falhas.length) {
  ...
  console.log(`  \x1b[33mA marca deixou de ser escolha de gente, ou passou a decidir o que um corpo É.\x1b[0m`);
  process.exit(1);
}
