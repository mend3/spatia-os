...
class Node {
  ...
  querySelector(sel) {
    ...
    const busca = (n) => {
      ...
      for (const f of n.children) { const achado = busca(f); if (achado) return achado; }
      return null;
    };
    return busca(this);
  }
  procurar(classe) {
    ...
    for (const f of this.children) { const a = f.procurar(classe); if (a) return a; }
    return null;
  }
...
const CORPUS = { collection: 'espatial_fixture', prefix: '', cwd: '/Users/victor/workspace/espatial-fixtures' };
...
if (cenario === 'pleno') {
  ...
  conferir('§1 TOPOLOGIA conta ARQUIVO, não corpos+luas',
    por('TOPOLOGIA')?.valor === '72 corpos', `saiu "${por('TOPOLOGIA')?.valor}"`);
  conferir('§1 nenhuma linha publica a soma corpos+luas',
    !linhas.some((l) => /\b(439|451|460)\b/.test(l.valor)),
    linhas.filter((l) => /\b(439|451|460)\b/.test(l.valor)).map((l) => l.rotulo).join(','));
  conferir('§1 as luas são contadas com o NOME delas',
    por('LUAS')?.valor === '367 em órbita · 12 seções sem espaço', `saiu "${por('LUAS')?.valor}"`);
  conferir('§1 corte de seção não sai como estado normal',
    por('LUAS')?.tom === 'warn', `tom "${por('LUAS')?.tom}"`);
  conferir('§1 a palavra "corpos" nomeia UMA grandeza na tela',
    linhas.filter((l) => /\bcorpos?\b/.test(l.valor)).length === 1);

  // ------------------------------------- §2 de que corpus é este céu
  conferir('§2 o corpus é declarado ANTES de tudo que ele qualifica',
    linhas[0].rotulo === 'CORPUS', `primeira linha "${linhas[0].rotulo}"`);
  conferir('§2 corpus sai como coleção · raiz',
    por('CORPUS')?.valor === 'espatial_fixture · espatial-fixtures', `saiu "${por('CORPUS')?.valor}"`);
  conferir('§2 prefixo VAZIO é escolha declarada e não vira segmento',
    !/·\s*·|·\s*$/.test(por('CORPUS')?.valor ?? ''));

  // ------------------------------------- §3 diagnóstico REAL, nunca uma sequência de OK
  conferir('§3 subsistema caído sai dizendo o que falta',
    por('SATÉLITE OPENAI')?.valor === 'requer OPENAI_API_KEY');
  conferir('§3 nenhuma linha afirma OK sem medida',
    !linhas.some((l) => /^(OK|ONLINE|READY|STABLE|PRONTO)$/i.test(l.valor)));
  conferir('§3 o resumo conta o que respondeu',
    status.textContent === 'todos os subsistemas respondendo', `saiu "${status.textContent}"`);

  // ------------------------------------- §4 os dois blocos, e nenhum vazio
  const grupos = root.grupos();
  conferir('§4 dois blocos NOMEADOS, na ordem céu → subsistemas',
    grupos.map((g) => g.titulo).join('>') === 'CÉU>SUBSISTEMAS', grupos.map((g) => g.titulo).join('>'));
  conferir('§4 nenhum bloco nasce vazio — cabeçalho sobre nada é moldura',
    grupos.every((g) => g.linhas > 0));
  conferir('§4 a topologia não mora no bloco dos subsistemas',
    por('TOPOLOGIA')?.grupo === 'CÉU' && por('MEMÓRIA VETORIAL')?.grupo === 'SUBSISTEMAS');

  // ------------------------------------- §5 a leitura do disco chega TARDE e reescreve
  conferir('§5 sondagem repetida REESCREVE a linha em vez de empilhar',
    linhas.filter((l) => l.rotulo === 'DISCO').length === 1,
    `${linhas.filter((l) => l.rotulo === 'DISCO').length} linhas de disco`);
  conferir('§5 o disco mostra a leitura mais NOVA',
    por('DISCO')?.valor === 'ÁRVORE LIMPA · NENHUM ANEL', `saiu "${por('DISCO')?.valor}"`);
  conferir('§5 o tom da timeline chega traduzido, não perdido',
    por('DISCO')?.tom === 'ok', `tom "${por('DISCO')?.tom}"`);

  // ------------------------------------- §7 a tela é UMA camada, e ela é a da frente
  conferir('§7 a entrada anuncia que está na frente', tela.estado().camada === 'boot');
  conferir('§7 a entrada não declara uma segunda camada',
    tela.estado().camadas.join('>') === 'mundo>boot', tela.estado().camadas.join('>'));

  // ------------------------------------- §8 o call site, no arquivo de verdade
  const main = src('src/main.js');
  conferir('§8 o retorno de loadGraph não é guardado para virar contagem',
    !/=\s*scene\.loadGraph\(/.test(main), 'alguém voltou a publicar o retorno de loadGraph');
  conferir('§8 o medidor CORPUS · arq recebe a contagem de ARQUIVO',
    /frame\.applyGraph\(corpos\)/.test(main));
  conferir('§8 a nota da timeline e a tela dizem o MESMO número',
    /TOPOLOGIA CARREGADA · \$\{corpos/.test(main));
  conferir('§8 a leitura do disco chega à tela de entrada',
    /boot\.disco\(texto, tom\)/.test(main));
  ...
  conferir('§8 o corpus vem do SERVIDOR, sem palpite',
    /corpus: graph\.corpus \?\? null/.test(main) && !/boot\.ceu\([^)]*'/.test(main));
  conferir('§8 nenhuma segunda camada de entrada é montada',
    !/createSplash|hud\/splash\.js/.test(main));
}
...
if (cenario === 'degradado') {
  ...
  conferir('§6 topologia que não carregou sai em vermelho, com o motivo',
    por('TOPOLOGIA')?.valor === 'indisponível — servidor não respondeu' && por('TOPOLOGIA')?.tom === 'bad',
    `saiu "${por('TOPOLOGIA')?.valor}" (${por('TOPOLOGIA')?.tom})`);
  conferir('§6 topologia que não carregou NUNCA vira zero',
    !/\b0\b/.test(por('TOPOLOGIA')?.valor ?? ''));
  conferir('§6 corpus não declarado sai em vermelho, nunca em branco',
  ...
  conferir('§6 memória não medida não vira "0 chunks"',
    por('MEMÓRIA VETORIAL')?.valor === 'OFFLINE' && por('MEMÓRIA VETORIAL')?.tom === 'bad');
  conferir('§6 o resumo NOMEIA quantos degradaram',
    /^1 subsistema\(s\) degradado\(s\)/.test(status.textContent), `saiu "${status.textContent}"`);
}

if (cenario === 'vazio') {
  const { boot, por } = montar();
  boot.ceu({ corpos: 0, luas: { shown: 0, dropped: 0 }, corpus: CORPUS });
  conferir('§6 céu medido e VAZIO é outro fato, e também é vermelho',
    por('TOPOLOGIA')?.valor === 'VAZIA' && por('TOPOLOGIA')?.tom === 'bad',
    `saiu "${por('TOPOLOGIA')?.valor}" (${por('TOPOLOGIA')?.tom})`);
  conferir('§6 lua MEDIDA e ausente é zero, e zero é uma medida',
    por('LUAS')?.valor === '0 em órbita' && por('LUAS')?.tom === 'ok');
  conferir('§6 corpus declarado continua declarado num céu vazio',
    por('CORPUS')?.tom === 'ok');
}
...
if (cenario === 'gate') {
  ...
  conferir('§9 o gate oferece DUAS respostas legítimas',
    fileira?.children.length === 2, `${fileira?.children.length} botões`);
  conferir('§9 nenhuma delas é "aperte qualquer tecla"',
    !/QUALQUER TECLA|PRESS ANY KEY/i.test(fileira?.textContent ?? ''));
  conferir('§9 a resposta em SILÊNCIO também entra', await (async () => {
  ...
  conferir('§9 a camada sai da pilha e o que estava embaixo reaparece',
    tela.estado().camada === 'mundo', tela.estado().camada);
