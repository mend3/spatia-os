...
globalThis.Date = class extends DateReal {
  constructor(...args) {
    super(...(args.length ? args : [agoraFalso]));
  }
  static now() {
    return agoraFalso;
  }
...
const avancar = (ms) => {
  agoraFalso += ms;
  tiques.forEach((fn) => fn());
};

const tiques = [];
globalThis.setInterval = (fn) => {
  tiques.push(fn);
  return tiques.length;
};
...
conferir('§1 ☠️ antes da primeira leitura a idade é travessão, não `AGORA`', idade() === '—', idade());
conferir('§1 e nada está vencido — não medi ≠ medi e venceu', vencida() === undefined);
conferir(
  '§1 o tique de 1s não inventa idade sozinho',
  (() => {
    avancar(10 * AFERICAO_MS);
    return idade() === '—' && vencida() === undefined;
  })(),
  idade()
);
...
conferir('§3 ⚠️ e o ponto NÃO mudou — quem envelheceu foi a leitura, não o serviço', brilho(1) === 'on');

// ---------------------------------------------------- §4 a leitura vence, e a tela declara
conferir(
  '§4 (controle) uma aferição perdida ainda não vence',
  (() => {
    agoraFalso = 1_700_000_000_000;
    frame.applyHealth(saude());
    avancar(AFERICAO_MS + 1000);
    return tom() === undefined && vencida() === undefined;
  })(),
...
conferir(
  '§4 ☠️ e o GRUPO declara que parou de afirmar o presente',
  vencida() === 'vencida',
  String(vencida())
);
conferir(
  '§4 ⚠️ o que foi medido NÃO é apagado — sai a certeza, não o fato',
  brilho(1) === 'on',
  String(brilho(1))
);
...
conferir(
  '§5 o vencimento persiste enquanto ninguém aferir',
...
conferir(
  '§6 ☠️ e o serviço que caiu aparece SEM ninguém ter perguntado',
  brilho(1) === 'off',
  String(brilho(1))
);
...
agoraFalso -= 5_000;
avancar(0);
conferir('§7 ⚠️ relógio para trás lê AGORA, e não a distância absoluta', idade() === 'AGORA', idade());
conferir(
  '§7 e não declara vencida uma leitura que acabou de acontecer',
...
conferir('§8 `AFERICAO_MS` é exportado por `hud/frame.js`', /export const AFERICAO_MS/.test(fonteFrame));
conferir(
  '§8 ☠️ `main.js` IMPORTA o período em vez de repetir o número',
  /import \{[^}]*AFERICAO_MS[^}]*\} from '\.\/hud\/frame\.js'/.test(fonteMain)
);
conferir(
  '§8 ☠️ e o laço de saúde não tem número literal',
  /setInterval\(poll, AFERICAO_MS\)/.test(fonteMain) &&
    !/setInterval\(poll, *[0-9_]+\)[\s\S]{0,80}api\.health/.test(fonteMain)
);
conferir(
  '§8 o vencimento é DERIVADO do período, não escolhido',
...
conferir(
  '§9 `server/ambient.py` continua declarando `SCAN_SECONDS`',
  Number.isFinite(scanSeconds),
  String(scanSeconds)
);
conferir(
  '§9 ⚠️ o período do cliente é o do vigia — transcrição em dia',
  AFERICAO_MS / 1000 === scanSeconds,
  `${AFERICAO_MS / 1000}s aqui × ${scanSeconds}s no vigia`
);

// ---------------------------------------------------- §10 três idades, e nenhuma é a outra
const fonteStreams = src('src/hud/streams.js');
conferir(
  '§10 ☠️ a rampa do AVISO continua em DIAS, e é dele',
  /IDADE_AMBAR_D = 3/.test(fonteStreams) && /IDADE_VERMELHA_D = 7/.test(fonteStreams)
);
conferir(
  '§10 ☠️ a rampa da AFERIÇÃO não usa aqueles símbolos',
  !/IDADE_AMBAR_D|IDADE_VERMELHA_D/.test(fonteFrame)
);
conferir(
  '§10 a célula ÍNDICE continua medindo DIAS do corpus, não a leitura',
  /headCell\('index', 'ÍNDICE'\)/.test(fonteFrame) && /const age = health\.index_age_days/.test(fonteFrame)
);
conferir(
  '§10 e o rótulo da aferição não empresta a palavra ÍNDICE',
...
for (const remonta of ['installProviders', 'showProviders', 'voice.applyHealth', 'api.units']) {
  conferir(`§11 ⚠️ o laço não remonta \`${remonta}\` — remontar não é aferir`, !laco.includes(remonta));
}
conferir(
  '§11 ☠️ o laço não emite no barramento — aviso repetido é o que ensina a não ler a tela',
  !/emit\(/.test(laco)
);
...
conferir(
  '§11 ☠️ o `catch` NÃO chama `applyHealth` — falha não carimba leitura',
  !/catch[\s\S]*applyHealth/.test(laco),
  laco.slice(laco.indexOf('catch'), laco.indexOf('catch') + 90)
);

// ---------------------------------------------------- §12 nada disso escreve na tela
conferir('§12 a moldura não importa `core/tela.js`', !/core\/tela\.js/.test(fonteFrame));
conferir(
  '§12 e não há camada, cena nem rota nova — a aferição não é um MODO',
...
conferir(
  '§13 ☠️ e o grupo vencido tem regra — senão o atributo não vira pixel',
...
conferir(
  '§14 a tabela de fontes tem os cinco pontos',
  declarado.size === pontos().length,
 ...
 * ela não marca, não.
 */
const corpoApply = fonteSrc.slice(
  fonteSrc.indexOf('applyHealth(health) {'),
...
for (const [id, fonte] of declarado) {
  ...
  conferir(
    `§14 \`${id}\` declara ${fonte} e ${aferido ? 'É' : 'NÃO é'} marcado por applyHealth`,
    aferido === (fonte === 'AFERICAO'),
    `declarado ${fonte}, marcado em applyHealth: ${aferido}`
  );
}

conferir(
  '§14 ☠️ o vencimento no CSS alcança SÓ a fonte que a aferição mediu',
...
conferir(
  '§14 vencido o grupo, `stream` e `graph` mantêm fonte fora da regra',
  porFonte.local === 'local' && porFonte.carga === 'carga',
  JSON.stringify(porFonte)
);
...
conferir(
  '§15 ☠️ o dono de `/api/health` é `main.js`, e mais ninguém sonda',
  intrusos.length === 0,
  `também chamam: ${intrusos.join(', ')}`
);
...
for (const arquivo of Object.keys(FORA_DO_DONO)) {
  ...
  const temCadencia =
    /setInterval\([\s\S]{0,400}?health\(\)/.test(fonte) ||
...
const blocoAbout = indexSrc.slice(
  indexSrc.indexOf("id: 'sys-about'"),
  indexSrc.indexOf("id: 'sys-services'")
);
...
for (const arquivo of ARQUIVOS_SRC.filter((f) => /aoAferir\(/.test(semProsa(src(f))))) {
  ...
  conferir(
    `§15 toda assinatura em \`${arquivo}\` guarda o cancelamento`,
    anonimos === 0,
    `${anonimos} assinatura(s) com o cancelamento descartado`
  );
  for (const nome of nomes) {
    conferir(
      `§15 e \`${nome}\` é chamado na destruição`,
      new RegExp(`destroy:[\\s\\S]{0,200}?\\b${nome}\\b`).test(fonte),
      arquivo
    );
  }
}
