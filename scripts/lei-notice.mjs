...
class N {
  ...
  append(...ns) { for (const n of ns) { n.parent?.remove?.call?.(n); n.parent = this; this.children.push(n); } }
...
globalThis.setInterval = (...args) => { timersVivos += 1; return setIntervalReal(...args); };
globalThis.clearInterval = (id) => { timersVivos -= 1; return clearIntervalReal(id); };
...
const aviso = (extra) => bus.emit({
  t: 'notice', topic: 'index', severity: 'warn', label: 'ÍNDICE VENCIDO',
  detail: '12 dias', action: 'reindexe a coleção `espatial_fixture`', at: agora(), ...extra,
});
...
conferir('§2 ☠️ o `action` chega ao pixel',
  telaDosAvisos().includes('reindexe a coleção `espatial_fixture`'), telaDosAvisos());
...
bus.emit({ t: 'notice', topic: 'graphdb', severity: 'warn', label: 'NEO4J FORA', detail: 'sem resposta', action: 'não rode a cadeia', at: agora() });
...
aviso({ at: mesmoAt });
aviso({ at: mesmoAt });
conferir('§5 ☠️ reentrega do mesmo (topic, at) não redesenha o nó',
...
const dia = 86_400;
bus.emit({ t: 'notice', topic: 'credential', severity: 'alert', label: 'CREDENCIAL EXPIRADA', detail: 'github expirada', action: 'reautorize em `#/security`', at: agora() - 10 });
const recente = avisos().find((n) => texto(n).includes('CREDENCIAL'));
conferir('§6 aviso de dez segundos lê em segundos', idadeDe(recente).textContent === 'HÁ 10S',
  idadeDe(recente).textContent);
conferir('§6 e não carrega tom de idade', idadeDe(recente).dataset.tone === undefined);

bus.emit({ t: 'notice', topic: 'corpus', severity: 'warn', label: 'ALFA3D', detail: '', action: 'y', at: agora() - 3 * dia });
...
conferir('§6 ⚠️ e NÃO tem o mesmo peso visual do de dez segundos',
  idadeDe(tresDias).dataset.tone === 'warn' && idadeDe(recente).dataset.tone === undefined);

bus.emit({ t: 'notice', topic: 'topology', severity: 'warn', label: 'BETA9D', detail: '', action: 'y', at: agora() - 9 * dia });
...
conferir('§6 a idade nunca é negativa (relógio adiantado)',
  (() => { bus.emit({ t: 'notice', topic: 'other', severity: 'warn', label: 'GAMAFUTURO', detail: '', action: 'y', at: agora() + 5_000 });
...
bus.emit({ t: 'notice', topic: 'index', severity: 'info', label: 'ÍNDICE RENOVADO', detail: '0 dia(s) de idade', at: agora() });
conferir('§7 ☠️ `info` apaga o aviso de pé do MESMO tópico',
  !avisos().some((n) => texto(n).includes('ÍNDICE VENCIDO')), telaDosAvisos());
conferir('§7 e não toca nos outros tópicos', avisos().some((n) => texto(n).includes('NEO4J FORA')));
conferir('§7 `info` escreve UMA linha de timeline', linhas().length === linhasAntesDoInfo + 1);
conferir('§7 e a linha carrega o fato', texto(linhas()[0]).includes('ÍNDICE RENOVADO')
  && texto(linhas()[0]).includes('0 dia(s) de idade'), texto(linhas()[0]));

// ---------------------------------------------------- §8 o catálogo
bus.emit({ t: 'notice', topic: 'constructor', severity: 'warn', label: 'IMPOSTOR', detail: '', action: 'y', at: agora() });
const impostor = avisos().find((n) => texto(n).includes('IMPOSTOR'));
conferir('§8 ☠️ tópico da cadeia de protótipos não vira rótulo',
...
recusas.length = 0;
bus.emit({ t: 'notice', topic: 'index', severity: 'critical', label: 'A', detail: '', action: 'y', at: agora() });
conferir('§8 severidade fora do catálogo não desenha', avisos().length === antesDaRecusa);
conferir('§8 e a recusa diz por quê', recusas.some((m) => m.includes('fora do catálogo')));

recusas.length = 0;
bus.emit({ t: 'notice', topic: 'index', severity: 'alert', label: 'B', detail: '', at: agora() });
conferir('§8 ☠️ `alert` sem `action` não desenha — aviso sem o que fazer é ruído',
  avisos().length === antesDaRecusa);
conferir('§8 e a recusa nomeia o conserto', recusas.some((m) => m.includes('diga o que o operador faz')));

// ---------------------------------------------------- §9 a poda da timeline
const dePeAntes = avisos().length;
for (let i = 0; i < 40; i += 1) bus.emit({ t: 'query', text: `q${i}`, web: 0 });
conferir('§9 ☠️ a poda de 26 linhas NÃO come os avisos de pé', avisos().length === dePeAntes,
...
conferir('§11 nenhum outro módulo assina `notice`',
  [...src('src/main.js').matchAll(/on\('notice'/g)].length === 0);
conferir('§11 a HUD não importa a cena nem o kernel',
  !/from '\.\.\/(space|kernel)\//.test(fonte));

// ---------------------------------------------------- §12 o CSS existe para o que o JS escreve
const html = src('index.html');
for (const classe of ['avisos', 'timeline-rows', 'aviso', 'aviso-head', 'aviso-mark', 'aviso-topic',
  'aviso-label', 'aviso-age', 'aviso-detail', 'aviso-action']) {
  conferir(`§12 \`.${classe}\` tem regra no CSS`, new RegExp(`\\.${classe}[\\s,{:\\[]`).test(html));
}
conferir('§12 a severidade `alert` tem cor própria',
  /\.aviso\[data-severity="alert"\]/.test(html));
conferir('§12 a idade tem os dois degraus',
  /\.aviso-age\[data-tone="warn"\]/.test(html) && /\.aviso-age\[data-tone="bad"\]/.test(html));
conferir('§12 os degraus de idade são os MESMOS do cabeçalho (3d/7d)',
  /IDADE_AMBAR_D = 3/.test(fonte) && /IDADE_VERMELHA_D = 7/.test(fonte)
