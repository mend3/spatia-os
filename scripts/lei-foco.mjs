...
console.log(`${C.forte}§1  SEM PEDIDO, A MEMÓRIA MANDA${C.fim}`);
{
  ...
  checar('§1', desfecho === 'adiar' && pend.source === SALVO && aplicado.origem === ORIGEM.MEMORIA,
    'ninguém pediu nada: a sessão anterior volta, e a origem viaja como `memoria`');

  const g = criarFocoDeEntrada();
  checar('§1b', g.lembrar('', false) === 'sem-alvo' && g.pendente() === null,
    'memória vazia não agenda alvo nenhum');
  const h = criarFocoDeEntrada();
  checar('§1c', h.lembrar(SALVO, false) === 'sem-alvo' && h.pendente() === null,
...
console.log(`\n${C.forte}§2  PEDIDO ANTES DA MEMÓRIA${C.fim}  ${C.fraco}o app monta antes da topologia${C.fim}`);
{
  ...
  checar('§2', acao === 'adiar' && desfecho === 'cedeu' && alvo.source === PEDIDO,
...
console.log(`\n${C.forte}§3  PEDIDO DEPOIS DA MEMÓRIA${C.fim}  ${C.fraco}a topologia chega antes do app${C.fim}`);
{
  ...
  const alvo = f.consumir();
  checar('§3', naMemoria === SALVO && alvo.source === PEDIDO && alvo.origem === ORIGEM.PEDIDO,
...
console.log(`\n${C.forte}§3b  E ELA NÃO VOLTA DEPOIS DE UM PEDIDO JÁ APLICADO${C.fim}`);
{
  ...
  const desfecho = f.lembrar(SALVO, true);
  checar('§3b', acao === 'aplicar' && f.pendente() === null && desfecho === 'cedeu',
...
console.log(`\n${C.forte}§4  O PEDIDO ESPERA A POSIÇÃO${C.fim}`);
{
  ...
  checar('§4', semPos.pedir(PEDIDO, false) === 'adiar' && semPos.pendente().source === PEDIDO,
  ...
  checar('§4b', comPos.pedir(PEDIDO, true) === 'aplicar' && comPos.pendente() === null,
...
console.log(`\n${C.forte}§5  DESTRAVAR É PEDIDO${C.fim}`);
{
  ...
  const desfecho = f.lembrar(SALVO, true);
  checar('§5', acao === 'aplicar' && desfecho === 'cedeu' && f.pendente() === null,
...
console.log(`\n${C.forte}§6  MEMÓRIA QUE CEDEU NÃO VOLTA${C.fim}`);
{
  ...
  checar('§6', tentativas.every((t) => t === 'cedeu') && f.pendente().source === PEDIDO,
...
console.log(`\n${C.forte}§7  A ORIGEM CHEGA JUNTO COM O ALVO${C.fim}  ${C.fraco}é ela que decide a POSE${C.fim}`);
{
  ...
  checar('§7', mem.consumir().origem === ORIGEM.MEMORIA && ped.consumir().origem === ORIGEM.PEDIDO,
  ...
  checar('§7b', largado.source === SALVO && d.pendente() === null,
...
console.log(`\n${C.forte}§8  O ENDEREÇO PEDE FOCO E LEITURA${C.fim}`);
{
  ...
  checar('§8b', /ui\.focus-node/.test(ramo) && /handler\(\{\s*source:\s*ctx\.arg/.test(ramo),
...
console.log(`\n${C.forte}§9  A POSE GRAVADA É DA MEMÓRIA, NUNCA DO PEDIDO${C.fim}`);
{
  ...
  checar('§9', i >= 0 && /startOrbit\.(polar|distance)/.test(bloco),
...
console.log(`${C.ok}✓ a lei vale${C.fim}  ${C.fraco}pedido vence memória nos dois sentidos, o pedido `
