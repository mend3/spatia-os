...
class N {
  ...
  append(...ns) { for (const n of ns) { n.remove(); n.parent = this; this.children.push(n); } }
...
if (MUNDO === 'boot-com-fio') respostaDoBoot = {
...
const texto = (n) => (n ? [n.textContent, ...n.children.map(texto)].join(' ').replace(/\s+/g, ' ').trim() : '');
const acharClasse = (n, classe) => {
  if (n.className === classe) return n;
  for (const c of n.children) { const a = acharClasse(c, classe); if (a) return a; }
  return null;
};
...
const thread = (extra) => bus.emit({
  t: 'thread', continuity: 'resumed', thread: SESSAO, turn: 7, since: NASCEU, origin: CANAL, ...extra,
});

// ================================================================ o mundo do BOOT
if (MUNDO === 'boot-com-fio') {
  conferir('§B o fio que já existia é desenhado ANTES da primeira pergunta', Boolean(fioNode()),
    'nada na tela');
  conferir('§B ele diz que a PRÓXIMA pergunta sabe deste',
    fioTexto().includes('FIO ABERTO') && fioTexto().includes('A PRÓXIMA PERGUNTA SABE DESTE'), fioTexto());
  conferir('§B ⚠️ `turns` é CONTAGEM do que já houve, não a ordem da próxima',
    celula('row-meta') === '7 PERGUNTAS', celula('row-meta'));
  ...
  conferir('§B a pergunta seguinte reescreve o MESMO nó', avisos().filter((n) => n.dataset.thread !== undefined).length === 1);
  ...
  conferir('§1 ☠️ boot sem fio não escreve "SEM FIO" — não dizer nada é o que é verdade',
    avisos().length === 0 && linhas().length === 0);
  conferir('§1 o boot PERGUNTOU pelo fio', chamadas.some((c) => c.url === '/api/thread' && c.metodo === 'GET'),
    JSON.stringify(chamadas));

  // ---------------------------------------------------- §2 `resumed` chega ao pixel
  thread();
  conferir('§2 a continuidade chega ao pixel', fioTexto().includes('ESTA PERGUNTA SABE DAS ANTERIORES'), fioTexto());
  conferir('§2 o `turn` chega ao pixel como a ORDEM da pergunta', celula('row-meta') === '7ª PERGUNTA', celula('row-meta'));
  conferir('§2 o `since` chega ao pixel como a hora do fio', celula('row-time') === 'DESDE 14:02', celula('row-time'));
  conferir('§2 ⚠️ o `turn` não é chamado de TURNO — a palavra já é o laço interno do agente',
    !fioTexto().includes('TURNO') && !celula('row-meta').includes('TURNO'), fioTexto());
  conferir('§2 a sessão viaja no `title`, fora da régua da HUD', (fioLinha()?.title ?? '').includes(SESSAO), (fioLinha()?.title ?? ''));
  conferir('§2 a continuidade é legível como DADO', fioNode()?.dataset.thread === 'resumed');
  conferir('§2 o normal não sai em tom de alarme', !(fioLinha()?.className ?? '').includes('bad'), (fioLinha()?.className ?? ''));
  ...
  conferir('§4 ☠️ `broken` escreve UMA linha de timeline — a execução respondeu sem `error`',
    linhas().length === 1, `${linhas().length}`);
  conferir('§4 e a linha nomeia o que ACONTECEU', texto(linhas()[0]).includes('FIO PERDIDO')
    && texto(linhas()[0]).includes('O CLI NÃO ACHOU A SESSÃO ANTERIOR'), texto(linhas()[0]));
  conferir('§4 a linha sai em tom de falha', Boolean(linhas()[0]?.className.includes('bad')),
    linhas()[0]?.className ?? 'não há linha');
  conferir('§4 a condição diz o que passou a VALER, e não é a mesma frase',
    fioTexto().includes('ESTA PERGUNTA RODOU DO ZERO') && !fioTexto().includes('O CLI NÃO ACHOU'), fioTexto());
  conferir('§4 e ela salta aos olhos', (fioLinha()?.className ?? '').includes('bad'), (fioLinha()?.className ?? ''));
  conferir('§4 sem hora de nascimento, porque não há fio', celula('row-time') === '—', celula('row-time'));

  thread({ turn: 2 });
  conferir('§4 ☠️ a pergunta seguinte APAGA a condição — só a linha guarda que aquela resposta não teve memória',
    !fioTexto().includes('RODOU DO ZERO') && texto(linhas()[0]).includes('FIO PERDIDO'), fioTexto());
  ...
  conferir('§5 ⚠️ `new` e `none` não dizem a mesma coisa', frasedoNew !== fioTexto(), `${frasedoNew} | ${fioTexto()}`);
  conferir('§5 `new` promete a próxima', frasedoNew.includes('A PRÓXIMA PERGUNTA SABERÁ DESTA'), frasedoNew);
  conferir('§5 `none` diz que o cérebro não guarda conversa',
    fioTexto().includes('ESTE CÉREBRO NÃO GUARDA CONVERSA'), fioTexto());
  conferir('§5 ☠️ `new` CONTA a pergunta e `none` não conta nada (`null` ≠ `0`)',
    ordemDoNew === '1ª PERGUNTA' && celula('row-meta') === '', `${ordemDoNew} | ${celula('row-meta')}`);
  conferir('§5 nenhum dos dois é erro', !tomDoNew.includes('bad') && !(fioLinha()?.className ?? '').includes('bad'));
  conferir('§5 e nenhum dos dois escreve linha', linhas().length === 1, `${linhas().length}`);

  // ---------------------------------------------------- §6 o fio de ONTEM
  thread({ since: NASCEU - 86_400 });
  conferir('§6 ⚠️ um fio de ontem NÃO parece recém-aberto', /DESDE \d\d\/\d\d 14:02/.test(celula('row-time')),
    celula('row-time'));
  ...
  conferir('§7 a condição não vive entre as linhas',
  ...
  bus.emit({ t: 'notice', topic: 'index', severity: 'warn', label: 'ÍNDICE VENCIDO', detail: '12 dias',
    action: 'reindexe a coleção', at: Date.now() / 1000 });
  conferir('§8 o aviso de pé e o fio convivem no mesmo bloco', avisos().length === 2 && Boolean(fioNode()),
    `${avisos().length}`);
  conferir('§8 o fio vem primeiro — ele é sobre a pergunta que está sendo feita',
    avisos()[0].dataset.thread !== undefined);
  bus.emit({ t: 'notice', topic: 'index', severity: 'info', label: 'ÍNDICE RENOVADO', detail: '', at: Date.now() / 1000 });
  ...
  conferir('§9 e a recusa diz por quê', recusas.some((m) => m.includes('fora do catálogo')), recusas.join(' | '));
  recusas.length = 0;
  thread({ continuity: undefined });
  conferir('§9 continuidade AUSENTE também é recusa — carimbo que falta não é carimbo neutro',
    fioTexto() === antesDaRecusa && recusas.length === 1, fioTexto());

  // ---------------------------------------------------- §10 ☠️ `thread` não é `brain`
  conferir('§10 ☠️ o `thread` não escreve no estado do cérebro', store.snapshot().brain === null,
    JSON.stringify(store.snapshot().brain));
  conferir('§10 e não inventa campo no estado', store.snapshot().thread === undefined);
  conferir('§10 quem conta ferramenta continua lendo o `brain`',
    /on\('brain', \(event\) => set\(status, `\$\{event\.tools\}/.test(src('src/hud/permissions.js')));
  conferir('§10 nenhuma linha da timeline confundiu fio com núcleo',
    !linhas().some((n) => texto(n).includes('NÚCLEO')), 'NÚCLEO ONLINE saiu de um `thread`');

  // ---------------------------------------------------- §11 o botão que corta
  thread({ turn: 9 });
  conferir('§11 há botão quando há (ou haverá) fio', Boolean(fioBotao()));
  conferir('§11 e o rótulo diz o que ele faz', fioBotao()?.textContent === 'CORTAR O FIO',
  ...
  conferir('§11 o clique corta no SERVIDOR', Boolean(corte) && corte.url === '/api/thread', JSON.stringify(chamadas));
  conferir('§11 e nomeia a origem — um canal não corta o fio do outro', corte?.corpo?.origin === CANAL,
    JSON.stringify(corte?.corpo));
  conferir('§11 o corte escreve UMA linha (é instante, e foi o operador)',
    linhas().filter((n) => texto(n).includes('FIO CORTADO')).length === 1
    && texto(linhas()[0]).includes('FIO CORTADO'), texto(linhas()[0]));
  conferir('§11 a condição passa a dizer que a próxima começa do zero',
    fioTexto().includes('A PRÓXIMA PERGUNTA COMEÇA DO ZERO') && fioNode()?.dataset.thread === 'cut', fioTexto());
  conferir('§11 e não sobra botão para cortar o que não há', fioBotao() === null);

  thread({ turn: 9 });
  respostaDoCorte = { cut: 0, threads: [] };
  await fioBotao()?.handlers.click();
  conferir('§11 ☠️ `cut: 0` NÃO afirma corte — o que aconteceu sai do servidor, não do clique',
    texto(linhas()[0]).includes('NÃO HAVIA FIO A CORTAR'), texto(linhas()[0]));

  thread({ turn: 9 });
  respostaDoCorte = { cut: 0, threads: [{ origin: CANAL, session: SESSAO, turns: 9, since: NASCEU }] };
  await fioBotao()?.handlers.click();
  conferir('§11 fio que o servidor ainda declara continua na tela',
    fioNode()?.dataset.thread === 'resumed' && Boolean(fioBotao()), fioTexto());
  ...
  conferir('§12 e é o único em src/', (() => {
  ...
  conferir('§12 ⚠️ o fio não inventou classe nenhuma — nada de CSS novo', semCSS.length === 0, semCSS.join(', '));
  conferir('§12 o botão usa o primitivo, e não um fork', /button\(\{/.test(fonte) && /variant: 'outline'/.test(fonte));

  conferir('§13 nenhum handler do barramento estourou', !recusas.some((m) => m.includes('[bus]')),
    recusas.join(' | '));

  // ------------------------------------------------ o mundo do boot, num processo próprio
  const filho = spawnSync(process.execPath, [fileURLToPath(import.meta.url), 'boot-com-fio'], { encoding: 'utf8' });
  const linhasDoFilho = filho.stdout.split('\n').filter((l) => l.startsWith('  ✗') || /^\d+ leis/.test(l));
  conferir(`§B o mundo do BOOT (processo próprio) — ${linhasDoFilho[0] || 'sem saída'}`, filho.status === 0,
...
if (MUNDO === 'principal') console.log(`
